...
module Notifications
  class TestCase < ActiveSupport::TestCase
    def setup
      @old_notifier = AS::Notifications.notifier
      @notifier = AS::Notifications::Fanout.new
      AS::Notifications.notifier = @notifier
      @events = []
      @named_events = []
    ...
    def teardown
      AS::Notifications.notifier = @old_notifier
    end

  private

    def event(*args)
      AS::Notifications::Event.new(*args)
    end
  end
  ...
  class SubscribedTest < TestCase
    def test_subscribed
      ...
      AS::Notifications.subscribed(callback, name) do
        AS::Notifications.instrument(name)
        AS::Notifications.instrument(name2)
        AS::Notifications.instrument(name)
      end
  ...
  class InstrumentationTest < TestCase
    delegate :instrument, :to => AS::Notifications
    ...
    def test_instrumenter_exposes_its_id
      assert_equal 20, AS::Notifications.instrumenter.id.size
    end
